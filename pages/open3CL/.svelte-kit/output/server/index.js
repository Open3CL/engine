...
import {
  B as BINARY_FORM_CONTENT_TYPE,
  c as create_remote_key,
  p as parse_remote_arg,
  s as stringify,
  d as deserialize_binary_form,
...
async function load_server_data({ event, event_state, state, node, parent }) {
  ...
  const result = await record_span({
    ...
    fn: async (current2) => {
      const traced_event = merge_tracing(event, current2);
      const result2 = await with_request_store({ event: traced_event, state: event_state }, () =>
        load.call(null, {
          ...traced_event,
          fetch: (info, init2) => {
...
async function handle_remote_call_internal(event, state, options2, manifest, id) {
  ...
  try {
    ...
    const status =
      error2 instanceof HttpError || error2 instanceof SvelteKitError ? error2.status : 500;
    return json(
      /** @type {RemoteFunctionResponse} */
      {
        type: 'error',
...
async function render_page(
  ...
  try {
    ...
    return await render_response({
      ...
      fetched,
      data_serializer:
        ssr === false ? server_data_serializer(event, event_state, options2) : data_serializer
    });
  } catch (e) {
  ...
  const remote_id = get_remote_id(url);
  {
    ...
    if (remote_id) {
      ...
      const forbidden =
        ...
        (!request_origin || !options2.csrf_trusted_origins.includes(request_origin));
      if (forbidden) {
                      ...
                      (response2) => {
                        ...
                        resolve_span.setAttributes({
                          ...
                          'http.response.body.size':
                            response2.headers.get('content-length') || 'unknown'
                        });
                        return response2;
                      }
                    )
