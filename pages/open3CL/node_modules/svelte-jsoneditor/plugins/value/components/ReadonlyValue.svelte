<script lang="ts">import { isUrl } from '../../../utils/typeUtils.js';
import { createEditValueSelection } from '../../../logic/selection.js';
...
import { isCtrlKeyDown } from '../../../utils/keyBindings';
import { formatSize } from '../../../utils/fileUtils';
import Tag from '../../../components/controls/Tag.svelte';
const { path, value, mode, truncateTextSize, readOnly, normalization, parser, onSelect, searchResultItems } = $props();
let doTruncate = $state(true);
const isTruncated = $derived(doTruncate &&
    typeof value === 'string' &&
    value.length > truncateTextSize &&
    (!searchResultItems ||
        !searchResultItems.some((item) => item.active && item.end > truncateTextSize)));
const truncatedValue = $derived(isTruncated && typeof value === 'string' ? value.substring(0, truncateTextSize).trim() : value);
